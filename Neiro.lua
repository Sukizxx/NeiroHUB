...
-- FIX: Tombol Telegram & GitHub langsung di section, tanpa HStack
local SecSocial = TabInfo:Section({ Title = "Social" })
SecSocial:Button({ Title = "Telegram", Icon = "send", Desc = "Join grup: t.me/Nexsus009", Callback = function()
    local link = "https://t.me/Nexsus009"
    if setclipboard then setclipboard(link); Notify("Telegram", "Link dicopy: " .. link, 3)
    else ForceNotify("Telegram", "Copy manual: " .. link, 5) end
end })
SecSocial:Button({ Title = "GitHub", Icon = "github", Desc = "Repo: github.com/Sukizxx", Callback = function()
    local link = "https://github.com/Sukizxx"
    if setclipboard then setclipboard(link); Notify("GitHub", "Link dicopy: " .. link, 3)
...
if drawingAvailable then
    ...
    SecLine:Toggle({ Title = "Enable Line", Desc = "Tampilkan garis tracer", Value = false, Callback = function(val) lineEnabled = val; if not espEnabled and val then setMasterESP(true) end; if not val and not boxEnabled and not hologramEnabled then setMasterESP(false) end end })
    SecLine:Dropdown({ Title = "Line Mode", Desc = "Pilih posisi awal garis", Values = { "Top (dari atas kepala)", "Bottom (dari bawah kaki)" }, Value = "Bottom (dari bawah kaki)", Callback = function(val) lineMode = (val == "Top (dari atas kepala)") and "Top" or "Bottom" end })
    local SecBox = TabESP:Section({ Title = "ESP Box" })
    SecBox:Toggle({ Title = "Enable Box", Desc = "Tampilkan kotak", Value = false, Callback = function(val) boxEnabled = val; if not espEnabled and val then setMasterESP(true) end; if not val and not lineEnabled and not hologramEnabled then setMasterESP(false) end end })
    SecBox:Toggle({ Title = "Health Bar", Desc = "Bar kesehatan vertikal", Value = false, Callback = function(val) healthEnabled = val end })
    SecBox:Toggle({ Title = "Distance", Desc = "Jarak di bawah kaki", Value = false, Callback = function(val) distanceEnabled = val end })
    SecBox:Toggle({ Title = "Name", Desc = "Nama di atas kepala", Value = false, Callback = function(val) nameEnabled = val end })
    local SecHolo = TabESP:Section({ Title = "Hologram" })
    SecHolo:Toggle({ Title = "Enable Hologram", Desc = "Box transparan", Value = false, Callback = function(val) hologramEnabled = val; if not espEnabled and val then setMasterESP(true) end; if not val and not lineEnabled and not boxEnabled then setMasterESP(false) end end })
    local SecSettings = TabESP:Section({ Title = "Settings ESP" })
    SecSettings:Colorpicker({ Title = "ESP Color", Desc = "Pilih warna", Default = Color3.fromRGB(255,255,255), Callback = function(col) espColor = col end })
...
SecTeleport:Button({ Title = "Refresh Player List", Desc = "Refresh daftar player", Callback = function() refreshDropdown(); Notify("Player List","Daftar player direfresh",2) end })
...
SecServer:Button({ Title = "Copy My Server ID", Desc = "Copy ID server saat ini", Callback = function() if setclipboard then setclipboard(game.JobId); Notify("Server ID","Job ID dicopy!",3) else Notify("Server ID","setclipboard tidak tersedia",3) end end })
...
-- ========== INIT ==========
task.wait(0.1); Window:Show()

ForceNotify("Neiro GUI", "Loaded! Semua tab aktif.\nESP butuh Drawing API.\nFly: WASD + Spasi (naik) + Ctrl (turun).", 6)
